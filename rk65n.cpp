...
double RK65n::integrate(double t, N_Vector y, N_Vector yn_, NeuronModel &model, double dt) {
    realtype *yi, *yn;
    N_Vector Yi, Fi;
    yi = NV_DATA_S(y);
    yn = NV_DATA_S(yn_);
   ...
   for (k= 0; k < N; k++) {
       ...
       delta= fabs(yn[k]-y5[k]);
       if (delta > DBL_MIN) {
           //newdt= (pow((theEps)/(delta), 0.33))*dt;
...
void RK65n::swap(N_Vector y, N_Vector yout, int dim) {
    realtype *yi, *yn, tmp;
    yi = NV_DATA_S(y);
    yn = NV_DATA_S(yout);
    for(int i=0; i<dim; i++){
